...
var productListElem, customerListsElem, callbackDiv, searchBox,
...
setUp = function(){
    ...
    searchBox = document.getElementById('productSearch');

    // Drag and drop listeners. Using lib/dnd.js
    ...
    searchBox.addEventListener('input',
        function(e){
            ...
            if(searchTerm){
                ajaxGet('api/product/' + searchTerm, callback, 'searchResults', 'searchPerfomed', 'json');
            }
            else{
                document.dispatchEvent(new CustomEvent('productListChanged'));
            }
    });
    ...
    // List search results
    document.addEventListener('searchPerfomed',
        function(){
            var productList = upackStoredJSON('searchResults');
            // Build product list once data returned
            productListElem.innerHTML = '<h1>Search Results</h1>';
            // console.log(productList);

            if (productList === {}){
                console.info('No products match search term');
                return;
            }
            for (var i = 0; i < productList.length; i++) {
                createProductElem(productList[i], productListElem);
    ...
    document.addEventListener('customerListsDownloaded',
        function(){
            ...
            console.log('Redraw customer lists @productPage.php: anon event');
