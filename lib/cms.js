...
setUp = function(){
    ...
    document.addEventListener('productListUploaded', getAllProducts);

    // Update options for product selection elements (deletion and modification)
        // after new data retrieved from server
    document.addEventListener('productListDownloaded',
        function(){
            ...
            if (!productList){
                ...
                window.location.assign('createConfig.php');
                return;
            }
    ...
    document.getElementById('createForm').addEventListener('submit',
        function(event){
            ...
            ajaxPost('../api/product', data, callback, 'productList', 'productListUploaded', 'json');
    });
    ...
    document.getElementById('deleteForm').addEventListener('submit',
        function(event){
            ...
            ajaxDelete('../api/product/' + idToDelete, callback, 'deleteResponse', 'productListUploaded', 'text');
    ...
    document.getElementById('modifyForm').addEventListener('submit',
        function(event){
            ...
            ajaxPost('../api/product/modify', data, callback, 'productList', 'productListUploaded', 'text');
    });
...
getAllProducts = function(){
    ...
    ajaxGet('../api/product', callback, 'productList', 'productListDownloaded', 'json');
};
