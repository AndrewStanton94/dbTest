...
setUp = function(){
    ...
    buyButton.addEventListener('click', function(e) {
        ...
        if (e.target.textContent === 'Go to Details'){
            ...
            if (addressForm.reportValidity()){
                ...
                data.append('order', localStorage['customerLists']);
                ajaxPost('api/order', data, callback, 'orderResults', 'orderSubmitted', 'text');
            }
            // else{
                // Firefox: no text response;
                // Chrome: Feedback remains after error resolved. Is a Mozilla extension
            //     // If invalid, is it postcode? -> Give custom message
            //     var postcode = document.getElementById('postcode');
            //     if(postcode.validity.patternMismatch){
            //         postcode.setCustomValidity('Please give a UK postcode');
            //     }
            // }
        }
    });
    ...
    document.addEventListener('orderSubmitted',
        function(e) {
            var results = upackStoredJSON('orderResults');
            if (results === []){
                ...
                var order = results[1],
                    currentStock = results[2];

                if (confirm('We cannot suppy the requested number of ' + currentStock['prodName'] + ". Press Ok to continue order without this product. Cancel to modify manualy.")) {
                    ajaxDelete('api/customerList/' + order.prodId, callback, 'ajaxDeleteResponse', 'getAllLists', 'text');
                    buyButton.dispatchEvent('click');
                }
                else{
                    ajaxDelete('api/customerList/*', callback, 'ajaxDeleteResponse', 'getAllLists', 'text'); 
                }
            }
    });
